...
def reduce_dimension_ndc(df, ndc_df):
    ...
    '''
    df1 = pd.merge(df, ndc_df['Proprietary Name'], left_on='ndc_code', right_on=ndc_df['NDC_Code'])  
    df1.rename(columns={'Proprietary Name': 'generic_drug_name'}, inplace=True)
    return df1
...
def select_first_encounter(df):
    ...
    first_encounter_df = (first_encounter_df.drop_duplicates(subset=['encounter_id'], keep='first').drop_duplicates(subset=['patient_nbr'],
                          keep='first'))
...
def create_tf_numeric_feature(col, MEAN, STD, default_value=0):
    ...
    tf_numeric_feature = tf.feature_column.numeric_column(key=col, default_value = default_value, normalizer_fn=normalizer,
    dtype=tf.float64)
...
# Question 10
def get_student_binary_prediction(df, pred_mean):
    '''
    ...
    '''
    df['score'] = df[pred_mean].apply(lambda x: 1 if x>=5 else 0 )
